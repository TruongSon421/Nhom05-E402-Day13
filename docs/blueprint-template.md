...
- [BONUS_AUDIT_LOGS]: **+2đ** — Module `app/audit.py` ghi audit trail riêng tại `data/audit.jsonl` (tách khỏi `data/logs.jsonl`). Mỗi event gồm: ts (ISO UTC), action, user_id_hash (hashed – không PII), correlation_id, result, metadata. Được gọi trong mọi chat request và incident toggle. Kiểm tra: `cat data/audit.jsonl` sau khi chạy app. Evidence: `app/audit.py` + `data/audit.jsonl`.
